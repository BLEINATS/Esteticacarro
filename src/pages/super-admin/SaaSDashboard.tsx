...
export default function SaaSDashboard() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-slate-900">Visão Geral do SaaS</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <h3 className="font-bold text-slate-900 mb-6">Crescimento de MRR</h3>
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <h3 className="font-bold text-slate-900 mb-6">Evolução da Base</h3>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
      ...
      {/* Alerts Section */}
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        <h3 className="font-bold text-slate-900 mb-4">Alertas e Pendências</h3>
        <div className="space-y-3">
            {tenants.filter(t => t.status === 'suspended').map(t => (
                <div key={t.id} className="flex items-center justify-between p-4 bg-red-50 rounded-lg border border-red-100">
                    <div className="flex items-center gap-3">
                        <AlertTriangle className="text-red-600" size={20} />
                        <div>
                            <p className="font-bold text-red-900">{t.name} está suspenso</p>
                            <p className="text-xs text-red-700">Fatura em atraso desde {new Date(t.nextBilling).toLocaleDateString()}</p>
                        </div>
                    </div>
                    <button className="px-3 py-1.5 bg-white text-red-600 text-xs font-bold rounded border border-red-200 hover:bg-red-50">
                        Ver Detalhes
                    </button>
                </div>
            ))}
            {tenants.filter(t => t.status === 'trial').map(t => (
                <div key={t.id} className="flex items-center justify-between p-4 bg-blue-50 rounded-lg border border-blue-100">
                    <div className="flex items-center gap-3">
                        <CheckCircle2 className="text-blue-600" size={20} />
                        <div>
                            <p className="font-bold text-blue-900">{t.name} em período de teste</p>
                            <p className="text-xs text-blue-700">Trial encerra em {new Date(t.nextBilling).toLocaleDateString()}</p>
                        </div>
                    </div>
                    <button className="px-3 py-1.5 bg-white text-blue-600 text-xs font-bold rounded border border-blue-200 hover:bg-blue-50">
                        Oferecer Desconto
                    </button>
                </div>
            ))}
